...
def login():
    try:
    ##WebDriverWait(driver, 20).until(expected_conditions.presence_of_element_located((By.ID, 'loginAcc')))
    ...
    ##WebDriverWait(driver, 20).until(expected_conditions.element_to_be_clickable((By.ID, "btnLogin")))
    try:
        ...
        driver.get("https://ecssl.pchome.com.tw/sys/cflow/fsindex/BigCar/BIGCAR/ItemList")
        time.sleep(2)
        driver.find_element_by_id("backToShop")
        driver.close()
    except:
        ##登入失敗則強制報錯
        print("登入失敗強制報錯")
